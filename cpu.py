...
class ComputerBrain:
    def __init__(self, used_positions, circle_shape=None, x_shape=None, circle_group=None, user_movements=None):
        self.used_positions = used_positions
        self.x = x_shape
        ...
        self.available_positions = [pos for pos in self.game_grid if pos not in self.used_positions]
        ...
        if self.available_positions:
            ...
            if len(self.used_positions) < 2:
                ...
                for row in self.winning_positions:
                    pos_to_win = [pos for pos in row if pos not in self.used_positions]
                    if len(pos_to_win) == 1 and pos_to_win[0] not in self.used_positions:
                ...
                if not found_pos_to_win:
                    for row in self.winning_positions:
                        ...
                        if len(pos_to_block) == 1 and pos_to_block[0] not in self.used_positions:
                    ...
                    if not found_pos_to_block:
                        if len(self.own_mov) == 1 and [300, 300] in self.own_mov \
                            ...
                            for row in self.winning_positions:
                                positions_to_fill = [pos for pos in row if pos not in self.used_positions]
                                if len(positions_to_fill) == 2 \
                                        and all(pos not in self.used_positions for pos in positions_to_fill):
